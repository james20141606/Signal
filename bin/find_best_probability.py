...
parser = argparse.ArgumentParser(description='find best paras using new probability')
parser.add_argument('--save', dest='save_path',  default='bestparas_new/', type=str, help='save path')
parser.add_argument('--model', dest='model_path',default='models_new/',type=str, help='model path')
parser.add_argument('--l', dest='length',default=200,type=float, help='length')
...
if not os.path.exists(save_path):
    os.system('mkdir '+save_path[:-1]+str(args.length))
...
def get_probability(pred_xy,pi,mu,sigma,n_components,scaler,l=args.length):
    #print(scaler)
    real_xy=scaler.inverse_transform(pred_xy)
    real_xy_u=real_xy+l/2
    real_xy_l=real_xy-l/2
    prediction_xy_u=scaler.transform(real_xy_u)[0]
    prediction_xy_l=scaler.transform(real_xy_l)[0]
    p=0
    for i in range(n_components):
        x_u=scipy.special.erf((prediction_xy_u[0]-mu[i][0])/(1.414213562373*sigma[i][0]))
        x_l=scipy.special.erf((prediction_xy_l[0]-mu[i][0])/(1.414213562373*sigma[i][0]))
        y_u=scipy.special.erf((prediction_xy_u[1]-mu[i][1])/(1.414213562373*sigma[i][1]))
        y_l=scipy.special.erf((prediction_xy_l[1]-mu[i][1])/(1.414213562373*sigma[i][1]))
...
class visualize_report(object):
    ...
    def report(self,save=False):
        print('rmse',np.mean(self.rmseall))
        print('rmse',np.mean(self.rmseall_random))
        print('rmse-random', (np.mean(self.rmseall)-np.mean(self.rmseall_random)))
...
'''
python bin/find_best_probability.py --save bestparas_new_ran/ --model models_new/
'''
