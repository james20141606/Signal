...
parser.add_argument('--r', dest='repeat_count', default=10,type=int,help='repeat test counts')

args = parser.parse_args()
feature_counts = args.count
point_counts = args.index_count
test_counts = args.test_count
repeat_counts = args.repeat_count
...
def corelate_simple(features):
    ...
    for j in range(testnum):
        ...
        for i in range(batchsize):
            d[j][i]=distance[xn[j][i],yn[j][i]]
            r[j][i],_=relate(xn[j][i],yn[j][i],features)
            while np.isnan(r[j][i]):
                r[j][i],_=relate(np.random.randint(0,1500),np.random.randint(0,1500),features)
        re[j]=scipy.stats.pearsonr(d[j],r[j])[0]
    return -np.mean(re)
...
for rc in range(repeat_counts):
    weight_temp=np.zeros(500)
    for tc in tqdm(range(test_counts)):
        label=np.random.choice(500,replace=False,size=feature_counts,p=weight)
        pcc=corelate_simple(label)
        for fc in range(feature_counts):
            weight_temp[label[fc]] += pcc
    weight_temp=weight/np.sum(weight_temp)
...
large_label=np.zeros((test_counts,feature_counts))
for i in tqdm(range(test_counts)):
    label=np.random.choice(500,replace=False,size=feature_counts,p=weight)
    relation[i],nan_rate[i],x_total[i],y_total[i],r_total[i],u_total[i]=corelate(label)
    large_label[i]=label

index=np.argsort(-relation)
relation=relation[index]
large_label=large_label[index]
...
np.savetxt(path_model+'/label.txt',large_label,fmt='%d')
